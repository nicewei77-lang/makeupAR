...
- `docs/roadmaps/active/E7_LIP_BOUNDARY_PRE_AR_CALIBRATION_SPIKE_PLAN_KO.md`: active lip-first pre-AR boundary calibration plan. Use when the next task is to design or implement user-specific lip boundary generation before AR runtime; it now records the color/gradient downstream-use shortlist for calibration quality check, Best-of-N capture selection, retake reasons, makeup visibility, spill detection, the evidence-mapped user adjustment controls `cornerReach`, `upperLipTightness`, `lowerLipTightness`, and `verticalOffset`, the deferral of failure-mode classification to later eval, the clarified 11/12 flow where `captureSet` is an input ledger and `personalizedBoundary` is the actual mask/package handoff, the lockout for the rejected `manual_polygon_codex_visual_reference_v0` marking, the allowlist for `evidence/references/e7-user-gold-raw-20260626/`, Section 14B `Gold-Reference Mask Derivation Experiment` for overnight candidate generation/scoring/selection, M3A buildless runtime-sweep prep boundaries, and Section 24B `Overnight Multi-Agent Launch Contract` with the no-stop human-review override: missing human judgment becomes `auto_provisional|pending_review|partial`, not a stop condition. M1 now has Apple Vision, local/offline face parsing, buildless color/gradient evidence, user-painted gold raw references, provisional selected package, provisional adjustment, gate matrix, missing-scenario manifest, coordinate audit, reference review pack, and M3A draft prep, but ready still requires user-confirmed adjustment, new scenario captures, blendshape/face-state, coordinate/visibility exporter support, held-out eval, and split/exclusion/falloff acceptance evidence.
...
- `docs/roadmaps/active/E7_LIP_SAMPLE_PACK_V0_RUNTIME_REVIEW_CONTEXT_KO.md`: next-session context for the lip-only sample v0 runtime review; use before further lip sample tuning, HUD/slider fixes, lip boundary work, or cosmetic renderer implementation planning.
...
- For lip-first pre-AR boundary calibration, read `docs/roadmaps/active/E7_LIP_BOUNDARY_PRE_AR_CALIBRATION_SPIKE_PLAN_KO.md` after the E7.03 parent plan; keep heavy analysis in calibration/offline and keep AR runtime on lightweight ARFace UV/vertex tracking.
...
- Lip-first calibration path: M1 is still `partial`, not M2-ready, but all currently available buildless work has been advanced beyond the original Section 14B run. The current provisional package is `evidence/e7-lip-m1-packages/m1-lip-auto-selected-20260625T215327Z/`; use its `m1_gate_matrix.json`, `next_automation_queue.json`, and `m1_summary.json` as the next local entry point. The next true blockers are no longer “run more unattended agents” but missing evidence: user-confirmed adjustment, open-close/smile/pucker/yaw and held-out captures, blendshape/face-state export, named coordinate/viewport/clipW/depth/front-most visibility fields, coordinate audit pass, and accepted inner-mouth/corner/upper-lower behavior. `evidence/e7-lip-missing-scenario-captures/manifest-20260625T220600Z/capture_queue.md` is the next phone-connected capture queue. M3A draft artifacts exist under `evidence/e7-lip-runtime-sweep-prep/prep-20260625T215807Z/`, but M3B/runtime/Lip G/Y/R remain build-gated and blocked by incomplete M1/M2 inputs. Keep this lip-only, local-only, no upload, no live face parsing/Core ML runtime, and no UnityFramework/Xcode/iPhone build until explicitly approved.
