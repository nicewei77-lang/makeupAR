...
- E7 M1 Buildless Personalized Lip Package audit/repair is now complete for buildless evidence only and remains `partial`, not ready. The previous `m1-lip-package-20260625T112757Z` folder was audited as partial/not-ready, and the repaired output folder is `evidence/e7-lip-m1-packages/m1-lip-repair-20260625T120000Z/`. Mesh draft artifacts exist, but visual audit marks the mesh draft too broad for reference/gold. The reference mask is reproducible from the prior polygon JSON metadata, but `acceptedAsGold=false` and `reviewStatus=needs_user_review_before_gold`. Apple Vision lip contour extraction is implemented as a buildless M1 pre-filter signal; `evidence/e7-lip-m1-packages/m1-lip-apple-vision-20260625T132216Z/` records `visionContourAvailable=1`, confidence `1`, outer/inner contour point counts `14/6`, and a contour overlay for the same neutral capture frame. Local/offline face parsing is now downloaded and integrated for the same neutral capture as a silver M1 signal: `evidence/e7-lip-m1-packages/m1-lip-face-parsing-20260625T2255Z/` records `faceParsingSilver=1`, upper/lower lip label pixels `7325/9842`, inner-mouth label pixels `0`, a face parsing overlay, fused lip reference mask, and candidate masks where face parsing actually narrows generation. Color/gradient confidence is now implemented as a buildless PIL/numpy confidence-only gate and verified under `evidence/e7-lip-m1-packages/m1-lip-color-gradient-20260625T000000Z/`: neutral capture `colorGradientConfidence.status=computed`, `colorGradientComputed=1`, `lowContrastWarning=true`, `shadowWarning=false`, and `specularWarning=false`. New user adjustment observations record frequent insufficient horizontal width / missed corners, frequent lower-lip spill, possible upper-lip spill, and vertical-position dissatisfaction; the active plan therefore supersedes legacy `tightness` / `upperLowerBalance` / `cornerShrink` with `cornerReach`, `upperLipTightness`, `lowerLipTightness`, and `verticalOffset`. The buildless user adjustment gate is now implemented in code: `prepare_user_adjustment_candidates.py` creates 12 derived candidates/contact sheet/review template, `prepare_m1_lip_package.py --user-adjustment-review` applies only a valid `user_confirmed` selection to `lip-tight-user-v0`, and fusion/audit reject legacy-only or unconfirmed adjustment params. Failure-mode classification is intentionally deferred from the M1 ready gate until boundary quality reaches an initial useful level for edge-case taxonomy; missing failure-mode tags should be recorded as future eval follow-up, not as an M1 blocker. Verification used `/tmp` smoke outputs only; no retained package is promoted as real user-approved evidence. Remaining M1 required gates are still missing or partial: real user-confirmed adjustment evidence for the retained package, pucker/expression capture coverage, blendshape/face-state, accepted/gold reference mask, coordinate-space audit, visibility/front-most handling, held-out/eval plan, inner-mouth exclusion acceptance, corner falloff, and full upper/lower split acceptance. The repaired, Apple Vision, face parsing, color/gradient, and user-adjustment implementation checks all keep M1 `partial`; one-frame UV round-trip remains `same_frame_self_reconstruction` projection sanity only with `boundaryQualityProof=false`, `coordinateSpaceAuditStatus=pending`, `visibilityConfidence=partial`, and runtime variants not ready. This does not mark M1 ready, E7.3 Green, runtime readiness, or any M2/build gate.
...
- E7 M1 Buildless Personalized Lip Package v0: partial / honestly bounded. Audit, reproducible polygon-mask repair, fusion summary, and one-frame UV projection artifacts exist under `evidence/e7-lip-m1-packages/m1-lip-repair-20260625T120000Z/`. Apple Vision contour extraction is implemented and smoke-verified under `evidence/e7-lip-m1-packages/m1-lip-apple-vision-20260625T132216Z/`. Local/offline face parsing is integrated under `evidence/e7-lip-m1-packages/m1-lip-face-parsing-20260625T2255Z/` and now affects the fused reference mask plus `lip-tight-auto-v0`, `lip-tight-user-v0`, and `lip-safe-v0` candidate masks. Color/gradient confidence is implemented under `evidence/e7-lip-m1-packages/m1-lip-color-gradient-20260625T000000Z/` with `colorGradientComputed=1`, but it records `lowContrastWarning=true`. User adjustment tooling is implemented and smoke-verified with a temporary valid review: no-review audit keeps `userAdjustment_required_not_confirmed`, valid-review smoke removes user-adjustment failures while overall M1 remains `partial`. Failure-mode classification is deferred to future eval after the engine boundary reaches an initial useful quality level, so it no longer blocks M1. M1 is not ready because the remaining required gates are missing or partial: real retained user-confirmed adjustment evidence, pucker capture, blendshape/face-state, user-approved gold/reference mask, expression captures, coordinate-space audit, inner-mouth/corner/upper-lower acceptance, triangle visibility/front-most handling, and held-out/eval frame plan.
...
- M1 buildless lip package audit/repair evidence: `evidence/e7-lip-m1-packages/m1-lip-repair-20260625T120000Z/audit_report.json`, `audit_report.md`, `input_manifest.json`, `lip_mesh_draft.png`, `lip_mesh_draft_overlay.png`, `lip_mesh_draft_meta.json`, `lip_reference_mask.png`, `lip_reference_mask_meta.json`, `fusionSummary.json`, `fusionSummary.md`, `summary.json`, `summary.md`, `lip_probability.png`, `lip_coverage.png`, `lip_unknown.png`, `lip_debug_votes.png`, `lip_variants.json`, and `round_trip_overlay.png`. Apple Vision contour evidence is in `evidence/e7-lip-m1-packages/m1-lip-apple-vision-20260625T132216Z/apple_vision_lip_contour.json`, `apple_vision_lip_contour_overlay.png`, `fusionSummary.json`, and `m1_summary.json`. Local/offline face parsing evidence is in `evidence/e7-lip-m1-packages/m1-lip-face-parsing-20260625T2255Z/face_parsing_labels.json`, `face_parsing_lip_mask.png`, `face_parsing_upper_lip_mask.png`, `face_parsing_lower_lip_mask.png`, `face_parsing_inner_mouth_mask.png`, `face_parsing_overlay.png`, `face_parsing_confidence.json`, `lip_candidate_generation.json`, `fusionSummary.json`, and `m1_summary.json`; it records `status=silver`, upper/lower lip label pixels `7325/9842`, inner-mouth pixels `0`, reference mask reduction from `36431` to `18866` pixels, and face parsing influence on generated candidate masks. Decision is still `partial`; artifact existence, Vision availability, face parsing availability, and same-frame IoU are explicitly not success criteria.
...
- M1 repair remains partial: Apple Vision contour extraction and local/offline face parsing are implemented for the same neutral frame, but required M1 gates are still not implemented/run or not accepted for color/gradient confidence, user-confirmed adjustment, pucker/expression captures, blendshape/face-state, accepted gold/reference mask, coordinate-space audit, visibility/front-most handling, held-out/eval frame plan, inner-mouth exclusion acceptance, corner falloff, and full upper/lower split acceptance. Failure-mode classification is intentionally deferred until boundary quality is strong enough for meaningful edge-case taxonomy. The mesh draft overlay is visibly too broad and cannot be promoted to reference/gold; the polygon-derived reference mask is reproducible but not user-approved gold; the face parsing run is a closed-mouth silver signal with `inner_mouth=0`, so it cannot by itself prove inner-mouth exclusion or pucker behavior; same-frame round-trip IoU is projection sanity only, not boundary quality.
...
- Lip-first calibration path: M1 audit/repair is partial and the next boundary is M1-only repair, not M2. Apple Vision contour extraction, local/offline face parsing, color/gradient confidence, and buildless user-adjustment tooling now exist for the same neutral capture frame, but the exact next step is running/recording the remaining required M1 signals for the retained package: real user-confirmed adjustment selection, pucker/expression captures, blendshape/face-state, user-approved gold/reference mask, coordinate-space audit, visibility/front-most handling, held-out/eval frame plan, inner-mouth exclusion acceptance, corner falloff, and upper/lower split acceptance. Failure-mode classification is deferred to later eval after boundary quality improves and is no longer a current M1 blocker. If any current required M1 signals remain unsolved, keep M1 `partial` or `blocked`; do not proceed to runtime/M2/build work and do not count artifact existence, Vision availability, face parsing availability, user-adjustment tooling availability, or same-frame round-trip IoU as boundary quality success. Keep this lip-only, local-only, no raw-frame long-term storage, no upload, no live face parsing/Core ML runtime, and no UnityFramework/Xcode/iPhone build until explicitly approved.
