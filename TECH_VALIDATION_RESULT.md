...
- Lip-first calibration path: M1 audit/repair is partial and the next boundary is M1-only repair, not M2. Apple Vision contour extraction, local/offline face parsing, color/gradient confidence, buildless user-adjustment tooling, and user-painted gold raw references now exist. The exact next step is Section 14B `Gold-Reference Mask Derivation Experiment`: generate candidates from the user-painted gold set, score them, and produce `selected_policy.json` with `ready_for_coordinate_pairing|partial_needs_review|blocked`. Section 24B now defines the overnight multi-agent launch contract: current thread as Manager, Evidence + Dev first, then Scoring, Tester, and optional M3A Prep; no internal token cap, stop by 2026-06-26 12:00 KST / usage exhaustion / completion / build gate / honest partial-blocked result. After Section 14B, continue the remaining M1 signals for a traceable `personalizedBoundary`: coordinate-paired selected gold/reference mask, real user-confirmed adjustment selection, pucker/expression captures, blendshape/face-state, coordinate-space audit, visibility/front-most handling, held-out/eval frame plan, inner-mouth exclusion acceptance, corner falloff, and upper/lower split acceptance. Failure-mode classification is deferred to later eval after boundary quality improves and is no longer a current M1 blocker. If any current required M1 signals remain unsolved, keep M1 `partial` or `blocked`; do not proceed to runtime/M2/build work and do not count artifact existence, Vision availability, face parsing availability, user-adjustment tooling availability, raw gold availability, or same-frame round-trip IoU as boundary quality success. The active E7 lip plan now splits M3 into M3A buildless runtime-sweep prep and M3B build-gated runtime sweep; phone-disconnected agents may only do M3A artifacts such as candidate registry drafts, candidate/package/adjustment wiring, sweep evidence schema, contact-sheet/review checklist, static checks, and prior-evidence summaries, all marked draft or blocked unless M1/M2 outputs are truly ready. M3A cannot install candidates, collect runtime evidence, or decide Lip G/Y/R. Keep this lip-only, local-only, no upload, no live face parsing/Core ML runtime, and no UnityFramework/Xcode/iPhone build until explicitly approved.
