...
- Lip-first calibration path: M1 audit/repair is partial and the next boundary is M1-only repair, not M2. Apple Vision contour extraction and local/offline face parsing now exist for the same neutral capture frame, and face parsing affects candidate generation, but the exact next step is implementing/running the remaining required M1 signals for the same capture set: color/gradient confidence, user-confirmed adjustment, failure-mode classification, pucker/expression captures, blendshape/face-state, user-approved gold/reference mask, coordinate-space audit, visibility/front-most handling, held-out/eval frame plan, inner-mouth exclusion acceptance, corner falloff, and upper/lower split acceptance. If any of those remain unsolved, keep M1 `partial` or `blocked`; do not proceed to runtime/M2/build work and do not count artifact existence, Vision availability, face parsing availability, or same-frame round-trip IoU as boundary quality success. Keep this lip-only, local-only, no raw-frame long-term storage, no upload, no live face parsing/Core ML runtime, and no UnityFramework/Xcode/iPhone build until explicitly approved.
