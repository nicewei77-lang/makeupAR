...
- E7 lip-first pre-AR boundary calibration Phase 3: Phase 3 UV Projection preparation ready. One-frame execution is pending a real `fusionSummary.json`, same-moment capture pair, and accepted screen-space lip reference mask. This remains lip-only and buildless until an explicit build gate is approved.
...
- Lip-first calibration path: Phase 3 UV Projection preparation ready from the fixed Phase 2 `fusionSummary` output contract. The next buildless boundary is actual one-frame execution with a generated `fusionSummary.json`, same-moment `frame.png` / `arface_export.json`, and accepted screen-space lip reference mask. If any input is missing, record `partial` or `blocked` with the exact missing input instead of generating fake artifacts. Keep this lip-only, local-only, no raw-frame long-term storage, no upload, no live face parsing/Core ML runtime, and no UnityFramework/Xcode/iPhone build until explicitly approved.
