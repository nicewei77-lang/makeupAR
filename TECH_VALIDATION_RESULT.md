...
- `docs/roadmaps/active/E7_LIP_BOUNDARY_PRE_AR_CALIBRATION_SPIKE_PLAN_KO.md`: active lip-first pre-AR boundary calibration plan. Use when the next task is to design or implement user-specific lip boundary generation before AR runtime; M1 now has Apple Vision plus local/offline face parsing evidence, but ready still requires color/gradient, user-confirmed adjustment, failure-mode type, pucker, blendshape/face-state, accepted/gold reference, coordinate/visibility, held-out/eval, and split/exclusion/falloff acceptance evidence.
