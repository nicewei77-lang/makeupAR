...
- E7 M1 Buildless Personalized Lip Package audit/repair is now complete for buildless evidence only and remains `partial`, not ready. The previous `m1-lip-package-20260625T112757Z` folder was audited as partial/not-ready, and the repaired output folder is `evidence/e7-lip-m1-packages/m1-lip-repair-20260625T120000Z/`. Mesh draft artifacts exist, but visual audit marks the mesh draft too broad for reference/gold. The reference mask is reproducible from the prior polygon JSON metadata, but `acceptedAsGold=false` and `reviewStatus=needs_user_review_before_gold`. All M1 pre-filter/reference signals are now required for ready: Apple Vision contour, local/offline face parsing, color/gradient confidence, user-confirmed adjustment, failure-mode type, pucker capture, blendshape/face-state, accepted/gold reference mask, coordinate-space audit, visibility/front-most handling, held-out/eval plan, inner-mouth exclusion, corner falloff, and upper/lower split. The current repaired output records these missing gates explicitly, so `fusionSummary.json` caps all candidates at `partial_contract_only`. One-frame UV round-trip ran as `roundTripKind=same_frame_self_reconstruction` with `boundaryQualityProof=false`, `phase3ExecutionStatus=partial`, `coordinateSpaceAuditStatus=pending`, `visibilityConfidence=partial`, and all `lip_variants.json` entries `runtimeReady=false`. This does not mark M1 ready, E7.3 Green, runtime readiness, or any M2/build gate.
...
- Lip-first calibration path: M1 audit/repair is partial and the next boundary is M1-only repair, not M2. The exact next step is implementing/running every required M1 signal for the same capture frame: Apple Vision contour, local/offline face parsing, color/gradient confidence, user-confirmed adjustment, failure-mode classification, pucker capture, blendshape/face-state, user-approved gold/reference mask, coordinate-space audit, visibility/front-most handling, held-out/eval frame plan, inner-mouth exclusion, corner falloff, and upper/lower split. If any of those remain unsolved, keep M1 `partial` or `blocked`; do not proceed to runtime/M2/build work and do not count artifact existence or same-frame round-trip IoU as boundary quality success. Keep this lip-only, local-only, no raw-frame long-term storage, no upload, no live face parsing/Core ML runtime, and no UnityFramework/Xcode/iPhone build until explicitly approved.
