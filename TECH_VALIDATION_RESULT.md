...
- E7 M1 Buildless Personalized Lip Package audit/repair is now complete for buildless evidence only and remains `partial`, not ready. The previous `m1-lip-package-20260625T112757Z` folder was audited as partial/not-ready, and the repaired output folder is `evidence/e7-lip-m1-packages/m1-lip-repair-20260625T120000Z/`. Mesh draft artifacts exist, but visual audit marks the mesh draft too broad for reference/gold. The reference mask is reproducible from the prior polygon JSON metadata, but `acceptedAsGold=false` and `reviewStatus=needs_user_review_before_gold`. Apple Vision lip contour extraction is now implemented as a buildless M1 pre-filter signal; `evidence/e7-lip-m1-packages/m1-lip-apple-vision-20260625T132216Z/` records `visionContourAvailable=1`, confidence `1`, outer/inner contour point counts `14/6`, and a contour overlay for the same neutral capture frame. Remaining M1 required gates are still missing or partial: local/offline face parsing, color/gradient confidence, user-confirmed adjustment, failure-mode type, pucker/expression capture coverage, blendshape/face-state, accepted/gold reference mask, coordinate-space audit, visibility/front-most handling, held-out/eval plan, inner-mouth exclusion, corner falloff, and upper/lower split. The repaired output and Apple Vision package both keep M1 `partial`; one-frame UV round-trip from the repaired package ran as `roundTripKind=same_frame_self_reconstruction` with `boundaryQualityProof=false`, `phase3ExecutionStatus=partial`, `coordinateSpaceAuditStatus=pending`, `visibilityConfidence=partial`, and all `lip_variants.json` entries `runtimeReady=false`. This does not mark M1 ready, E7.3 Green, runtime readiness, or any M2/build gate.
...
- E7 M1 Buildless Personalized Lip Package v0: partial / honestly bounded. Audit, reproducible polygon-mask repair, fusion summary, and one-frame UV projection artifacts exist under `evidence/e7-lip-m1-packages/m1-lip-repair-20260625T120000Z/`. Apple Vision contour extraction is now implemented and smoke-verified under `evidence/e7-lip-m1-packages/m1-lip-apple-vision-20260625T132216Z/`, but M1 is not ready because the remaining required gates are missing or partial: local/offline face parsing, color/gradient confidence, user-confirmed adjustment, failure-mode type, pucker capture, blendshape/face-state, user-approved gold/reference mask, expression captures, coordinate-space audit, inner-mouth/corner/upper-lower rules, triangle visibility/front-most handling, and held-out/eval frame plan.
...
- `docs/roadmaps/active/E7_LIP_BOUNDARY_PRE_AR_CALIBRATION_SPIKE_PLAN_KO.md`: active lip-first pre-AR boundary calibration plan. Use when the next task is to design or implement user-specific lip boundary generation before AR runtime; M1 ready now requires Apple Vision, local/offline face parsing, color/gradient, user-confirmed adjustment, failure-mode type, pucker, blendshape/face-state, accepted/gold reference, coordinate/visibility, held-out/eval, and split/exclusion/falloff evidence.
...
- M1 repair remains partial: Apple Vision contour extraction is implemented for the same neutral frame, but required M1 gates are still not implemented/run or not accepted for local/offline face parsing, color/gradient confidence, user-confirmed adjustment, failure-mode classification, pucker/expression captures, blendshape/face-state, accepted gold/reference mask, coordinate-space audit, visibility/front-most handling, held-out/eval frame plan, inner-mouth exclusion, corner falloff, and upper/lower split. The mesh draft overlay is visibly too broad and cannot be promoted to reference/gold; the polygon-derived reference mask is reproducible but not user-approved gold; same-frame round-trip IoU is projection sanity only, not boundary quality.
...
- Lip-first calibration path: M1 audit/repair is partial and the next boundary is M1-only repair, not M2. Apple Vision contour extraction now exists for the same neutral capture frame; the exact next step is implementing/running the remaining required M1 signals for the same capture set: local/offline face parsing, color/gradient confidence, user-confirmed adjustment, failure-mode classification, pucker/expression captures, blendshape/face-state, user-approved gold/reference mask, coordinate-space audit, visibility/front-most handling, held-out/eval frame plan, inner-mouth exclusion, corner falloff, and upper/lower split. If any of those remain unsolved, keep M1 `partial` or `blocked`; do not proceed to runtime/M2/build work and do not count artifact existence, Vision availability, or same-frame round-trip IoU as boundary quality success. Keep this lip-only, local-only, no raw-frame long-term storage, no upload, no live face parsing/Core ML runtime, and no UnityFramework/Xcode/iPhone build until explicitly approved.
