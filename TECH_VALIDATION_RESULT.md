...
- M1 repair remains partial: Apple Vision contour extraction and local/offline face parsing are implemented for the same neutral frame, but required M1 gates are still not implemented/run or not accepted for color/gradient confidence, user-confirmed adjustment, failure-mode classification, pucker/expression captures, blendshape/face-state, accepted gold/reference mask, coordinate-space audit, visibility/front-most handling, held-out/eval frame plan, inner-mouth exclusion acceptance, corner falloff, and full upper/lower split acceptance. The mesh draft overlay is visibly too broad and cannot be promoted to reference/gold; the polygon-derived reference mask is reproducible but not user-approved gold; the face parsing run is a closed-mouth silver signal with `inner_mouth=0`, so it cannot by itself prove inner-mouth exclusion or pucker behavior; same-frame round-trip IoU is projection sanity only, not boundary quality.
