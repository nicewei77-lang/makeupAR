...
def lip_gloss(mask: Image.Image) -> Image.Image:
    ...
    height = max(float(ys.max() - ys.min()), 1.0)
    grid_y, grid_x = np.indices(alpha.shape)
    lower_bias = np.exp(-(((grid_y - (center_y + height * 0.14)) / (height * 0.16)) ** 2))
    horizontal = np.exp(-(((grid_x - center_x) / (width * 0.34)) ** 2))
    gloss = alpha * lower_bias * horizontal
...
def make_lip_style_atlas(variants: dict[str, Image.Image]) -> Image.Image:
    ...
    gloss = multiply(lip_gloss(full).filter(ImageFilter.GaussianBlur(radius=0.8)), 0.9)
