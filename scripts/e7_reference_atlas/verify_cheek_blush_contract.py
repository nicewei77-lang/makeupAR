...
def main() -> None:
    ...
    require(calibration_by_mask["cheek-session-mask-3-v1"]["densityGain"] == 2.05, "under-eye/session-3 must read clearly as an under-eye-to-cheek gradient without becoming a hard block")
    ...
    require(calibration_by_mask["cheek-session-mask-4-v1"]["densityGain"] == 1.7, "sunkissed1/session-4 must keep the selected M down 3 cheek patch readable at the shared slider")
    require(calibration_by_mask["cheek-session-mask-4-v1"]["centerGain"] == 0.36, "sunkissed1/session-4 must keep the nose tint visible without over-boosting the bridge")
