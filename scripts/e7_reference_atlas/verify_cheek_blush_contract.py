...
def main() -> None:
    ...
    require(calibration_by_mask["cheek-session-mask-3-v1"]["uvTransform"] == [1.18, 1.92, 0.0, 0.03], "under-eye/session-3 must sweep strongly from below-eye into upper-cheek, not stay in the tear-triangle")
    require(calibration_by_mask["cheek-session-mask-3-v1"]["densityGain"] == 2.35, "under-eye/session-3 must stay clearly visible even when the source drawing is very soft")
    ...
    require(calibration_by_mask["cheek-session-mask-4-v1"]["uvTransform"] == [1.72, 1.24, 0.0, -0.006], "sunkissed1/session-4 must grow from the face line toward the cheek instead of floating as small cheek dots")
    require(calibration_by_mask["cheek-session-mask-4-v1"]["partUvTransform"] == [1.95, 1.7, 0.0, -0.02], "sunkissed1/session-4 must enlarge the central nose component enough to read on the nose")
    ...
    require(calibration_by_mask["cheek-session-mask-4-v1"]["densityGain"] == 1.18, "sunkissed1/session-4 must keep enough cheek density after expanding toward the face line")
    require(calibration_by_mask["cheek-session-mask-4-v1"]["centerGain"] == 2.6, "sunkissed1/session-4 must make central nose density visibly applied")
