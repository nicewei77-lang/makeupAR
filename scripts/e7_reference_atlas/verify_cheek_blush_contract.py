...
def main() -> None:
    ...
    require(calibration_by_mask["cheek-session-mask-3-v1"]["uvTransform"] == [1.28, 1.72, 0.0, -0.015], "under-eye/session-3 must use the user-selected up A placement below the eye and fade down into upper cheek")
    require(calibration_by_mask["cheek-session-mask-3-v1"]["densityGain"] == 1.35, "under-eye/session-3 must stay balanced with the other cheek styles at the same slider")
    ...
    require(calibration_by_mask["cheek-session-mask-4-v1"]["uvTransform"] == [1.14, 1.2, 0.0, 0.018], "sunkissed1/session-4 must follow the selected M down 3 blue guide: a cheekbone-lowered outer patch attached to the face line")
    require(calibration_by_mask["cheek-session-mask-4-v1"]["partUvTransform"] == [2.038, 0.981, 0.0, -0.037], "sunkissed1/session-4 must keep the selected low nose oval while the cheek patch moves down toward the cheekbone")
    require(calibration_by_mask["cheek-session-mask-4-v1"]["partBlend"] == 1.0, "sunkissed1/session-4 must fully enable component-aware resampling for the nose component")
    require(calibration_by_mask["cheek-session-mask-4-v1"]["densityGain"] == 0.76, "sunkissed1/session-4 must keep the selected M down 3 placement while matching the average same-slider color strength")
    require(calibration_by_mask["cheek-session-mask-4-v1"]["centerGain"] == 0.08, "sunkissed1/session-4 must keep the nose tint visible without rising faster than the other cheek styles")
